...
class EmailChangeViewModel @Inject constructor(private val userApiService: UserApiService,
                                               private val builder: AppDatabase) : ViewModel() {
    private val disposable = CompositeDisposable()

    val user: MutableLiveData<User?> = MutableLiveData<User?>().apply { value = null }
    val newEMail: MutableLiveData<String> = MutableLiveData<String>().apply { value = "" }
    ...
    fun emailChange() {
        ...
        userApiService.emailchange(user.value!!.id, newEMail.value!!, password.value!!).subscribeOn(Schedulers.io()).subscribe({
            Timber.d("EMailChangeResponse is ${it}")
            when(it.status){
